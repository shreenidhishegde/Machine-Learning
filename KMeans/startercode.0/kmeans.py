...
def get_center(generator, distance_to_closest_centroid):
    ...
    for index in range(len(distance_to_closest_centroid)):
        cum_prob += distance_to_closest_centroid[index]
        if cum_prob > p:
            return index
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        y = np.zeros(N)
        distances = np.sum([np.sum((x[y == i] - centroids[i]) ** 2) for i in range(self.n_cluster)]) / N

        iter_count = 0
        while iter_count < self.max_iter:
            iter_count += 1
            y = np.argmin(np.sum(((x - np.expand_dims(centroids, axis=1)) ** 2), axis=2), axis=0)
            distances_updated = np.sum([np.sum((x[y == i] - centroids[i]) ** 2) for i in range(self.n_cluster)]) / N
            if np.absolute(distances - distances_updated) <= self.e:
                break
